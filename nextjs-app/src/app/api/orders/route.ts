...
export async function GET(request: NextRequest) {
  ...
  const avgTicket = currentKpis.completed_orders > 0
    ? Math.round(currentKpis.revenue / currentKpis.completed_orders)
    : 0;

  const prevAvgTicket = prevYearKpis.completed_orders > 0
    ? Math.round(prevYearKpis.revenue / prevYearKpis.completed_orders)
    : 0;
  ...
  return NextResponse.json({
    kpis: {
      ...
      avgTicket: {
        value: avgTicket,
        yoyChange: calcYoYChange(avgTicket, prevAvgTicket),
      },
      pendingOrders: {
